...
def raw_train():
    ...
    new_df = pd.DataFrame()
    new_df["path"] = df["file"].apply(lambda x: "images/" + x.split('/')[1][:-4] + "/" + x.split('/')[-1].split('.')[0] + f".jpg")
    ...
    new_df.to_csv(f"data/raw_train.csv", index= False)

def modi_train():
    train = pd.read_csv("data/train.csv")
    path, width, height, labels = [],[],[],[]

    counter = {}
    limit = int(input())

    for i in range(len(train)):
        label = train.iloc[i]["class"]
        if label in counter.keys():
            counter[label] += 1
        else:
            counter[label] = 1
        if counter[label] <= limit:
            path.append(train.iloc[i]["path"])
            width.append(train.iloc[i]["width"])
            height.append(train.iloc[i]["height"])
            labels.append(train.iloc[i]["class"])

    train_modi = pd.DataFrame({"path": path,
                            "width": width,
                            "height": height,
                            "class": labels})

    train_modi.to_csv("data/modi_train.csv", index = False)